...
public class Sword : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        //UnityEngine.Debug.Log(m_MotionData.Direction.ToString());
    }

    Vector3 CompareDirection(Sword swordToCompare)
    {
        Vector3 comparedDirection = m_MotionData.Direction - swordToCompare.m_MotionData.Direction;
        return comparedDirection;
    }

    private void OnTriggerEnter(Collider other)
    {
        if (other.TryGetComponent<Sword>(out Sword externalSword))
        {
            UnityEngine.Debug.Log(CompareDirection(externalSword));
        }
    }
}